...
class MainWindow(Qt.QMainWindow):
    ...
    def __init__(self, path=None, parent=None, show=False, make_pngs=False, make_video=False):
        ...
        self.actor_resampled_glyth = None
        self.show_resampled_glyth = not self.show_pc3
        ...
        self.plotter.add_checkbox_button_widget(
            self.toogleResampledGlyph, size=25)
    ...
    def _create_gui(self, parent=None):
        ...
        self.plotter = QtInteractor(self.frame, shape=(
            1, 3), lighting="light kit")  # "none"
        # Force same view for both subplots
        self.plotter.renderers[2].camera = self.plotter.renderers[1].camera = self.plotter.renderers[0].camera
        ...
        toogleResampledGlyphButton = Qt.QAction(
            'Toogle Resampled Glyphs', self)
        toogleResampledGlyphButton.setShortcut('Ctrl+R')
        toogleResampledGlyphButton.triggered.connect(self.toogleResampledGlyph)
        fileMenu.addAction(toogleResampledGlyphButton)
    ...
    def set_point_size(self, val):
        ...
        if self.actor_pc3:
            self.actor_pc3.GetProperty().SetPointSize(val)
    ...
    def locate_files(self, path: Path):
        self.files = sorted(
            path.glob('SPHBody_ExcitationHeart_*.vtu'), key=_natural_sort_path_key)
        if self.files:
            self.fileformat = 'vtu'
            return
        self.files = sorted(
            path.glob('SPHBody_ExcitationHeart_*.npz'), key=_natural_sort_path_key)
        if self.files:
            self.fileformat = 'npz'
            return
        else:
    ...
    def reload_mesh(self):
        ...
        if self.actor_glyth:
            ...
            self.actor_glyth = None
        if self.actor_pc3:
            self.plotter.remove_actor(self.actor_pc3)
            self.actor_pc3 = None
        if self.actor_resampled_glyth:
            self.plotter.remove_actor(self.actor_resampled_glyth)
            self.actor_resampled_glyth = None
        ...
        cw = 0.95
        if self.show_pc3:
            self.actor_pc3 = self.plotter.add_mesh(self.point_cloud3,
                                                   render_points_as_spheres=True,
                                                   point_size=self.point_size,
                                                   lighting=True,
                                                   opacity=self.opacity,
    ...
    def load_dir(self, path: Path):
        ...
        if self.fileformat == 'vtu':
            d = pv.read(self.files[0])
            self.point_cloud = pv.PolyData(d.points)
            self.available_input_scalars = ['None'] + d.array_names
            ...
            raise NotImplemented(f"Unkown file format {self.fileformat}")
        ...
        self.point_cloud3 = pv.PolyData(self.point_cloud.points)
    ...
    def load_file(self, path: Path, input_scalar: str):
        if self.fileformat == 'vtu':
            ...
            if input_scalar == 'None':
                return d.points, None
            else:
                return d.points, d[input_scalar]
        elif self.fileformat == 'npz':
    ...
    def load(self, t):
        ...
        if self.show_pc3 is not None:
            self.point_cloud3.points = self.point_cloud.points
        ...
        if self.show_glyth:
            if t + 1 < len(self.files) and t > 0:
                points, vectors = self.load_file(self.files[t], 'Vector')

                pc = pv.PolyData(points[::self.GLYPH_ELEM_SKIP])
        ...
        if self.show_resampled_glyth:
            if t + 1 < len(self.files) and t > 0:
                _, points = self.load_file(self.files[t], 'ReXYZ')
                _, vectors = self.load_file(self.files[t], 'ReVector')

                pc = pv.PolyData(points[::self.GLYPH_ELEM_SKIP])
                pc.vectors = vectors[::self.GLYPH_ELEM_SKIP]
                glyph = pc.glyph(factor=self.GLYPH_SCALE_FACTOR)
                if self.actor_resampled_glyth is not None:
                    self.actor_resampled_glyth.GetMapper().SetInputData(glyph)
                else:
                    self.plotter.subplot(0, 2)
                    self.actor_resampled_glyth = self.plotter.add_mesh(
                        glyph, color='blue')
        else:
            self.plotter.remove_actor(self.actor_resampled_glyth)
            self.actor_resampled_glyth = None
    ...
    def toogleResampledGlyph(self, _):
        self.show_resampled_glyth = not self.show_resampled_glyth
        self.show_pc3 = not self.show_pc3
        self.reload_mesh()
