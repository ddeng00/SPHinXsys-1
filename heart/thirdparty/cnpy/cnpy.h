...
namespace cnpy {
    ...
    //void npz_save(std::string zipname, std::string fname, It begin, It end,
    ...
    //
    //  std::vector<char> npy_header = create_npy_header<It::value_type>(shape);
    //
    //  // Prepare buffer
    //  size_t nels = std::accumulate(shape.begin(), shape.end(), 1,
    //                                std::multiplies<size_t>());
    //  size_t nbytes = nels * sizeof(It::value_type) + npy_header.size();
    //
    //
    ...
    //  std::copy(begin, end, ptr + npy_header.size());
    //
    ...
    template <typename T, typename F> // std::function<void(char *)>
    void npz_save(std::string zipname, std::string fname, F&& copy_func,
                  const std::vector<size_t> &shape, std::string mode = "w",
                  int32_t comp = ZIP_CM_STORE) {
      libzip::flags_t flag = 0;
      if (mode == "a") {
    ...
    void npz_save(std::string zipname, std::string fname, const T *data,
      ...
      std::memcpy(ptr, npy_header.data(), npy_header.size());
      std::memcpy(ptr + npy_header.size(), data, nels * sizeof(T));
      //std::copy(data.begin(), data.end(), ptr + npy_header.size());
